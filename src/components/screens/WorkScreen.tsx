...
export function WorkScreen({ projects, lede, locale }: WorkScreenProps) {
  const dict = getDictionary(locale);
  return (
    <main className={styles.stage}>
      <SiteChrome locale={locale} active={dict.nav.work} fixed={false} />
      <div className={styles.layout}>
        <WorkPhraseAndGrid projects={projects} lede={lede} locale={locale} />
        <FooterLogo />
      </div>
    </main>
  );
